...
//        appointmentPage.emailTextboxRegP.sendKeys(appointment.getEmail());
    ...
    public void userClicksSigninButton() {
        Driver.clickWithJS(appointmentPage.SignInButton);


    }

    @Then("user clicks username TextBox at Signin page")
    public void userClicksUsernameTextBoxAtSigninPage() {
        Driver.clickWithJS(appointmentPage.Usernametextbox);

    }

    @And("user enters username at Signin page")
    public void userEntersUsernameAtSigninPage() {
        appointmentPage.Usernametextbox.sendKeys(appointment.getFirstname());
    }

    @Then("user clicks password button at Signinpage")
    public void userClicksPasswordButtonAtSigninpage() {
        Driver.clickWithJS(appointmentPage.Passwordtextbox);

    }

    @And("user enters password at Signin page")
    public void userEntersPasswordAtSigninPage() {
        appointmentPage.Passwordtextbox.sendKeys("12345678");
    }

    @Then("user clicks Signin button last")
    public void userClicksSigninButtonLast() {
        Driver.clickWithJS(appointmentPage.SigninButtonlast);
    }

    @Then("user clicks myPagesPatient button")
    public void userClicksMyPagesPatientButton() {
        Driver.wait(2);
        Driver.clickWithJS(appointmentPage.myPagesPatientbutton);
    }

    @Then("user clicks myAppointments button")
    public void userClicksMyAppointmentsButton() {
        Driver.clickWithJS(appointmentPage.myAppointmentbutton);
    }

    @Then("user gets Patient id")
    public void userGetsPatientId() {
         String PatId=Driver.waitAndGetText(appointmentPage.myPatientId,3);
        //String PatId=appointmentPage.myPatientId.getText();
        System.out.println(PatId);
    }

    private String PatId=Driver.waitAndGetText(appointmentPage.myPatientId,3);
    public String Id= PatId;
